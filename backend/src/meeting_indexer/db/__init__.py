...
def documents_with_status(conn: psycopg.Connection, status: str) -> list[tuple[str, str | None]]:
    """(rel_path, error) of documents with the given status."""
    return conn.execute(
        "SELECT rel_path, error FROM documents WHERE status = %s ORDER BY rel_path", (status,)
    ).fetchall()
...
def save_document(
    ...
    *,
    ...
    document_id = returned_id(
        conn.execute(
            """
        INSERT INTO documents (rel_path, sha256, file_type, page_count, status, error,
                               extractor_version, llm_model, indexed_at)
        VALUES (%(rel_path)s, %(sha256)s, %(file_type)s, %(page_count)s, %(status)s, %(error)s,
                %(extractor_version)s, %(llm_model)s, CASE WHEN %(status)s = 'indexed' THEN now() END)
